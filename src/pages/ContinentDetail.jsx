...
function ContinentDetail() {
    ...
    return (
        <div
            className="bg-slate-800 text-white min-h-screen w-full flex flex-col gap-4 items-center justify-center relative">
            ...
            <h1
                className="captitalize text-2xl font-bold drop-shadow-lg">Top Countries in {selectedContinent.name} for your next holiday</h1>

            <div className="select_country  flex items-center justify-start gap-6 flex-wrap">
                {selectedContinent.countries.map((country) => {
                    return (<div key={country.id} style={{ backgroundImage: `url(${country.image})` }}
                        onClick={() => { navigate(`/continent/${continentId}/country/${country.id}`) }}
